...
<head>
    ...
    <title>المدفوعات | منصة أمان</title>
    <style>
        /* Mobile Menu Toggle */
        .menu-toggle {
            display: none;
...
<body>
    <div class="container animate-fade-in">
        ...
        <div class="menu">
            <!-- Sidebar content same as medical-records.php -->
            <table class="menu-container" border="0">
                ...
                <tr class="menu-row">
                    <td class="menu-btn menu-icon-schedule menu-active menu-icon-schedule-active">
                        <a href="payments.php" class="non-style-link-menu non-style-link-menu-active"><div><p class="menu-text">المدفوعات</p></a></div>
                    </td>
                </tr>
                <tr class="menu-row">
            ...
            <div class="animate-slide-up delay-100">
                <table border="0" width="100%" style="border-spacing:0;margin:0;padding:0;margin-top:25px;">
                    <tr>
                        ...
                        <td>
                            <p class="heading-main12" style="margin-right:15px;">المدفوعات والعمليات</p>
                        </td>
                        <td width="15%">
                ...
                <center>
                    <div class="abc scroll" style="height:550px;width:95%;">
                        <table width="100%" class="sub-table scrolldown glass-card" border="0" style="border-collapse: collapse;">
                            <thead>
                                <tr>
                                    <th class="table-headin">رقم العملية</th>
                                    <th class="table-headin">تاريخ الحجز</th>
                                    <th class="table-headin">المبلغ</th>
                                    <th class="table-headin">الوسيلة</th>
                                    <th class="table-headin">الحالة</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $sql = "select payments.*, appointment.apponum, appointment.appodate from payments 
                                        inner join appointment on payments.appoid = appointment.appoid 
                                        where appointment.pid = ? order by payment_date desc";
                                ...
                                if($result->num_rows == 0){
                                    ...
                                    <p class="heading-main12" style="font-size:18px;color:#475569">لا توجد عمليات دفع مسجلة</p>
                                    </center><br><br><br><br></td></tr>';
                                } else {
                                    while($row = $result->fetch_assoc()){
                                        $status_class = '';
                                        if($row["payment_status"] == 'مكتمل') $status_class = 'status-confirmed';
                                        elseif($row["payment_status"] == 'معلق') $status_class = 'status-pending';
                                        elseif($row["payment_status"] == 'مرفوض') $status_class = 'status-cancelled';

                                        echo '<tr class="animate-fade-in">
                                            <td style="padding:20px;text-align:center;font-weight:700;">#PAY-'.$row["payment_id"].'</td>
                                            <td style="padding:20px;text-align:center;">'.$row["appodate"].' (#'.$row["apponum"].')</td>
                                            <td style="padding:20px;text-align:center;color:var(--primary);font-weight:700;">'.$row["amount"].' ر.ي</td>
                                            <td style="padding:20px;text-align:center;">'.$row["payment_method"].'</td>
                                            <td style="padding:20px;text-align:center;"><span class="status-badge '.$status_class.'">'.$row["payment_status"].'</span></td>
                                        </tr>';
                                    }
                                }
                                ?>
